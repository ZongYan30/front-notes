...
function deepProxy(obj) {
  return new Proxy(obj, {
    get(target, key) {
      console.log("调用了getter -key", key);
      if (isObject(target[key])) return deepProxy(target[key]);
      return target[key];
    },
    set(target, key, value) {
      console.log("调用了setter -key", key, value);
      target[key] = value;
      return true;
    },
  });
...
/** reactive响应式函数
 * @param {Object} param 参数需要是对象
 * @returns {Object} 返回代理对象
 */
function reactive(param) {
  return deepProxy(param);
}
...
arr.push(4); // 会被拦截,并触发很多拦截项，具体自己操作看下
```
