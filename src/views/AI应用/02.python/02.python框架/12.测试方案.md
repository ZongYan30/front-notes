...
```
test/              # 待新建各类业务单元用例
├── integration/             # 接口集成测试（当前可用、调试完成）
│   ├── __init__.py
│   ├── conftest.py          # 独立一套：建测试库、alembic、启动FastAPI 18000
│   └── test_categories.py   # 分类CRUD接口集成用例（12条全部可通过）
├── e2e/                     # 端到端多业务流程测试（刚修复路径问题）
│   ├── __init__.py
│   ├── conftest.py          # 独立一套初始化（刚改完路径，和integration重复逻辑）
│   └── ...                  # 待写完整业务串联E2E用例
├── unit/                    # 单元测试目录：测单个函数/Service逻辑，Mock DB
└──   └── test_category_schemas.py   # 待新建各类业务单元用例
```
