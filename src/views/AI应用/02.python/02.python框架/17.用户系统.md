...
- 业务问题
  ...
  - 模型层：想不到啥问题
  - 业务层：
    - 密码怎么保存才能防止泄露后造成灾难性后果？ (数据库存储hash过的密码)
    - 用户登录成功后颁发什么样的凭证才是可信的？ (使用jwt)
      - 用户凭证的过期时间在哪里设置？      (放在系统设置中了,可以通过接口api设置)
      - 用户凭证的密钥在哪里配置？        (一般放在环境变量中，发版的时候改一下)
    - 是否需要定义新的异常？              (要定义新异常AuthException)
  - API层：
    - 如何知晓当前登录的用户？
    - 如何限制接口必须登录后才能访问？
      - 修改访问权限后对应的测试脚本需要修改
...
from zy_python_project.model.base import Base, IDMixin, TimestampMixin
...
from zy_python_project.core.config import web_setting
from zy_python_project.exception.auth import AuthException
from zy_python_project.exception.database import DatabaseException
from zy_python_project.model.setting import Setting
from zy_python_project.model.user import User
from zy_python_project.schema.user import (
    TokenResponse,
    UserLogin,
    UserRegister,
    UserResponse,
)
from zy_python_project.service.base import BaseService
...
from zy_python_project.service.user_service import UserService
