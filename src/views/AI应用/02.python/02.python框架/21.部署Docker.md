...
![image-20260702135701716](https://zongyan30.github.io/front-assets/agent/docker.png)
...
# 先执行迁移，再启动 FastAPI 服务
CMD ["sh", "-c", "alembic upgrade head && uvicorn zy_python_project.main:app --host 0.0.0.0 --port 8080"]
```
...
4. 启动镜像
   ...
   ```shell
    ...
    -e WEB_CORS_ORIGINS="http://localhost,http://127.0.0.1,http://localhost:5173,http://127.0.0.1:5173" `
    -e WEB_CORS_EXPOSE_HEADERS=X-Process-Time `
    -e WEB_JWT_SECRET_KEY=8f7e1a2b3c4d5e6f7a8b9c0d1e2f3a4b `
    -e DB_HOST=192.168.31.229 `
    -e DB_PORT=5432 `
