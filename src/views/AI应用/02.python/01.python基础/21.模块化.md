...
│       ├── module_a.py # 模块a
│       ├── module_b.py # 模块b
│       ├── __main__.py # 包入口文件(命令行直接运行会执行)
│       └── __init__.py # 子包初始化文件(导入的时候会执行)
└── main.py  # 主程序入口
```
...
在类中定义 `__slots__ = (属性名列表)`,表示这个类的实例只允许拥有指定的这些属性，不再为每个实例创建 **dict**，改用固定长度的数组存储属性。

核心效果：

- 大幅减少内存占用（实例数量巨大时提升明显，节省 30%\~50% 内存）
- 限制动态新增属性，不在 slots 里的属性无法赋值，直接报错

```python
class User:
    # 声明实例仅允许 name、age 两个属性
    __slots__ = ("name", "age")
...
# 1. 无法新增未声明的属性
u.gender = "女"  # AttributeError: 'User' object has no attribute 'gender'
...
**保留动态属性**
如果想要 **slots** 省内存，又偶尔需要动态新增属性，在 slots 里加上 **dict**：
...
u = User("小李", 22)
u.phone = "123456" # 可以新增属性，但内存优化效果减弱
```
