...
![我的](https://zongyan30.github.io/front-assets/小兔鲜/member_picture_1.png)
...
> 分包技巧：
>
> 分包主要是放置一些用户点击频率不高的页面，这样就是可以让用户在需要这个页面的时候才进行加载。这样可以优化小程序的加载速度。
>
> 可以按照模块分包，比如现在我有一个“会员模块”,我就可以建立一个分包--pagesMember 文件夹，然后关于该模块的页面都放置在这个文件夹下。
>
> 然后再配置分包预下载即可。
...
![新建分包页面](https://zongyan30.github.io/front-assets/小兔鲜/member_picture_2.png)
...

{
  ...
  // 分包预下载规则
  "preloadRule": {
    //在“我的”页面打开完之后，会在任何环境下都下载子包pagesMember下的资源。
    "pages/my/my": {
      "network": "all", // [!code ++]
...
![登录状态](https://zongyan30.github.io/front-assets/小兔鲜/member_picture_3.png)
...
![会员信息页](https://zongyan30.github.io/front-assets/小兔鲜/member_picture_4.png)
...
> // 上传文件需要使用 uni.uploadFile(),之前已经做过拦截了，调用这个 API 之后，会自动拦截加上 token

![更新头像](https://zongyan30.github.io/front-assets/小兔鲜/member_picture_5.png)
...
/*
此处注意:
1.模板中v-model是不可以出现可选链的赋值，否则TS报错。
<input class="input" type="text" placeholder="请填写昵称" v-model="profile.nickname" />
2.如果写成断言的形式，那么运行的时候就会报错
const profile = ref<ProfileDetail>()
v-model="profile!.nickname"
报错:无法找到undefined上的nickname属性,这是因为vue在解析完数据之后，渲染模板的时候，此时profile是undefined,所以模板报错。
3.解决办法就进行断言
...
const onSubmit = async () => {
  ...
  const res = await putMemberProfileAPI({
    ...
    birthday,
    profession,
    //此处加undefined的原因是当刷新页面，这个时候就是默认的["","",""],传给后端就会出错的，不传的话，后端就不会修改为""
