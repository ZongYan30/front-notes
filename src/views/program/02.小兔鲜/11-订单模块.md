...
![填写订单](https://zongyan30.github.io/front-assets/小兔鲜/order_picture_1.png)
...
![导航栏交互](https://zongyan30.github.io/front-assets/小兔鲜/order_picture_2.gif)
...
// 动画必须在页面渲染完毕，绑定动画效果,相当于onMounted
onReady(() => {
  // 动画效果,导航栏背景色
...
const getOrderDetail = async () => {
  ...
  //只有在订单状态为待收货，待评价，已完成时，可获取物流信息。
  /* 如过此处写if判断,会显得比较繁琐，用数组的includes方法是不错的选择 */
  if (
    [
      OrderStatus.WATTING_RECEIEVE,
      OrderStatus.WATTING_COMMENT,
...
export enum OrderStatus {
  ...
  CANCEL = 6,
          ...
          <!-- 待评价/已完成/已取消 状态: 展示删除订单
          此处使用枚举类的判断，因为枚举类本职上是number类型
          -->
          <view
            v-if="orderDetail?.orderState >= OrderStatus.WATTING_COMMENT"
            class="button delete"
            @click="onDelete"
          >
            删除订单
...
![订单列表](https://zongyan30.github.io/front-assets/小兔鲜/order_picture_3.png)
...
```ts
//支付
const onPayment = async (item: OrderItem) => {
  if (import.meta.env.DEV) {
    //开发环境下模拟支付
    await payMockApi({ orderId: item.id });
  } else {
    //正式环境下支付接口--需要特定的企业级的appid
    const res = await paymentApi({ orderId: item.id });
    await wx.requestPayment(res.result); //调用wx支付接口，参数是后端返回的参数配置项
  }
  uni.showToast({ icon: "success", title: "支付成功" });
  item.orderState = OrderStatus.WATTING_RECEIEVE;
};
```
