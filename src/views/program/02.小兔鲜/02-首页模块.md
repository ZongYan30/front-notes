...
> Q:为什么从外界传入list而不是直接在子组件内部获取数据？
>
> A:因为封装组件的话，一般UI是通用的，其次在项目中，可能不止一处使用了该组件。所以从外部传入数据更加通用。
...
>
> 因为首页会有下拉刷新的功能，到时候需要多个模块进行数据联系，如果在子组件中，那么通信就变的复杂了。另外，组件封装的标准化就是传递props,React也是如此思想。
...
> 此处有几个需要注意的点：
>
> 1.子调用父的时候,需要使用组件上的ref,但是一般我们定义组件上的ref，并不会指定组件的类型，这就很痛苦。我们可以借助TS的InstanceType<>类型工具,来获取到组件的实例。
>
> 2.scroll-view组件上是具有 @scrollToLower事件的,借助该事件可以实现页面触底功能。
...
- 配置 `refresher-triggered` 属性，关闭下拉状态,即下拉的loading
...
>
> 1.重置猜你喜欢的状态:页码,猜你喜欢list数据,是否滚动到底部的isFnish数据.
>
> 2.在首页的下拉刷新执行函数onRefresherrefresh中,父调子获取组件内部数据。
...
2.生成骨架屏之后的代码会在小程序开发工具中当前文件下，有两个文件分别是wxml和wxss

3.将这两个文件的内容阅读并且升级为vue组件

4.在pages/index/components/skeleton.vue中,将微信小程序代码修改为uni-app的vue代码。
