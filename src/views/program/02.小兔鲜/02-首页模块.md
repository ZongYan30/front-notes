...
![自定义导航栏](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_1.png)
...
![安全区域](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_2.png)
...
![轮播组件](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_3.png)
...
> Q:为什么从外界传入 list 而不是直接在子组件内部获取数据？
>
> A:因为封装组件的话，一般 UI 是通用的，其次在项目中，可能不止一处使用了该组件。所以从外部传入数据更加通用。
...
![1677150782440](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_4.png)
...
3. 设置首页底色为 `#F7F7F7`
   ...
   --设置的话需要在父组件中 Index.vue 中

   ```css
   <style lang='scss'>
   //这里不可以加scoped 要不然不起作用的，page相当于页面的body
   page{
...
> 为什么该组件不复用，却还是要在父组件中请求数据，然后传值给子组件呢？
>
> 因为首页会有下拉刷新的功能，到时候需要多个模块进行数据联系，如果在子组件中，那么通信就变的复杂了。另外，组件封装的标准化就是传递 props,React 也是如此思想。

热门推荐功能，后端根据用户的消费习惯等信息向用户推荐的一系列商品，前端负责展示这些商品展示给用户。

**参考效果**

![热门推荐](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_5.png)
...
![猜你喜欢](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_6.png)
...
**设置 page 和 scroll-view**

```vue
<template>
  <NavBar />
  <scroll-view class="scroll-view" scroll-y>
    <YanSwiper :list="bannerList" />
    <Cate />
    <Hot :list="hotList" />
    <YanGuess />
  </scroll-view>
...
.scroll-view {
  flex: 1; //竖向的scroll-view必须设置高度，此处的高度按照flex来占满了
}
</style>
...
//该API在猜你喜欢的组件中调用,不再采用外面传递props的形式，因为不止这一个页面使用该组件，为了避免别的地方的麻烦，所以在组件内部请求数据是最优解。
```

**类型声明**

通用分页结果类型如下，新建 `src/types/global.d.ts` 文件：放置整个项目要用的通用类型
...
//对于分页的数据，后端返回的结果只有items字段不一致，我们可以使用泛型来进行类型定义
```
...
2. 父滚动触底需加载分页
3. 组件通讯，子调父
4. 判断是否在最后一页了，是的话就不进行触底请求了，否则会浪费性能的。
...
> 此处有几个需要注意的点：
>
> 1.子调用父的时候,需要使用组件上的 ref,但是一般我们定义组件上的 ref，并不会指定组件的类型，这就很痛苦。我们可以借助 TS 的 InstanceType<>类型工具,来获取到组件的实例。
>
> 2.scroll-view 组件上是具有 @scrollToLower 事件的,借助该事件可以实现页面触底功能。
...
import { ref } from "vue";
// 获取猜你喜欢组件实例
const guessRef = ref<XtxGuessInstance>(); //指定组件的实例类型
...
```ts
import YanSwiper from '@/components/YanSwiper/YanSwiper.vue'
import YanGuess from '@/components/YanGuess/YanGuess.vue'

declare module 'vue' {
  export interface GlobalComponents {
    YanSwiper: typeof YanSwiper
    YanGuess: typeof YanGuess
  }
}

// 通用的组件实例类型
export type GuessInstance = InstanceType<typeof YanGuess>
//借助TS工具获取组件实例
typeof YanGuess 获取的类型是defineComponent() 该函数的返回值是组件的构造函数
InstanceType<构造函数类型>  返回构造函数的类型,所以GuessInstance类型就是组件YanGuess的实例类型
```
...
// 分页参数--使用TS工具Required<对象>将参数都变为必选
...
- 配置 `refresher-triggered` 属性，关闭下拉状态,即下拉的 loading

### 参考代码

猜你喜欢组件定义重置数据的方法

> 因为在首页中，猜你喜欢的模块是组件内部维护数据的，当下拉刷新的时候，需要重新获取猜你喜欢的数据:
>
> 1.重置猜你喜欢的状态:页码,猜你喜欢 list 数据,是否滚动到底部的 isFnish 数据.
>
> 2.在首页的下拉刷新执行函数 onRefresherrefresh 中,父调子获取组件内部数据。
...
//解释:为什么这里要用Promise.all()
/* 为了优化性能，如果使用await的话会影响性能。
    // await getBannerList()
  	// await getHot()
  	// await getCategory()
  	这样的话只能在第一个结束玩之后才开始第二个，如果用户的网速是比较慢的情况下，就需要等待很久。
  	
  	Promise.all()可以同时让异步操作开始执行，但是必须都成功之后才会返回成功状态。
  	这样的话，下拉刷新的时候，三个请求就会同时发送，不用顺序执行的。
  */
</script>
...
![骨架屏](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_7.png)
...
![骨架屏](https://zongyan30.github.io/front-assets/小兔鲜/home_picture_8.png)

1.在微信开发者工具中，点击生成骨架屏按钮

2.生成骨架屏之后的代码会在小程序开发工具中当前文件下，有两个文件分别是 wxml 和 wxss

3.将这两个文件的内容阅读并且升级为 vue 组件

4.在 pages/index/components/skeleton.vue 中,将微信小程序代码修改为 uni-app 的 vue 代码。
...
onLoad(async () => {
  isloading.value = true//加载数据的时候开启骨架屏
  await Promise.all([getBannerList(), getHot(), getCategory()])
  isloading.value = false//数据加载完关闭骨架屏
})
</Script>
```
